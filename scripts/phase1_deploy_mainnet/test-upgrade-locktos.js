...
async function main() {
    ...
    let tx = await lockTosContract.connect(adminLockTos).upgradeTo(LockTOSv2Proxy);
    await tx.wait();
    ...
    let tx1 = await lockTosProxyContract.connect(adminLockTos).setImplementation2(LockTOSv2Logic0, 0, true);
    await tx1.wait();
    ...
    let tx2 = await lockToslogicContract.connect(adminLockTos).setStaker(stakingProxyAddress);
    await tx2.wait();
