...
import {IDTOSSnapshot} from "../interfaces/IDTOSSnapshot.sol";
    ...
    IDTOSEvent
{
        ...
        if (rebaseIntervalSecond > 0 && compoundInteresRatePerRebase > 0) {
            ...
            if ( (lastRebaseTime == 0 && total > 0) || total == 0) {
                ...
                if(period > 0){
                    ...
                    uint256 oldFactor = getFactor();
                    uint256 newFactor = _calcNewFactor(total, addAmount, oldFactor);
                    ...
                    rebaseTotal++;
                    uint256 rebaseIndex = rebaseTotal;
                    emit OnRebase(rebaseIndex, newFactor, addAmount, addAmount-total, block.timestamp);
                }
            }
        }
    }
    ...
    function _factorAt(uint256 snapshotId) internal view virtual returns (bool, uint256, uint256) {
        ...
        LibDTOSSnapshot.FactorSnapshots storage snapshots = factorSnapshots;
    ...
    function _valueAt(uint256 snapshotId, LibDTOSSnapshot.BalanceSnapshots storage snapshots, address account) internal view
    ...
    function _updateBalanceSnapshots(
            LibDTOSSnapshot.BalanceSnapshots storage snapshots,
            uint256 balances,
        ...
        if (index < currentId) {
            ...
            snapshots.balances[snapshots.ids.length-1] = balances;
        }
    ...
    function _updateFactorSnapshots(
        LibDTOSSnapshot.FactorSnapshots storage snapshots,
        uint256 factors,
        ...
        if (_lastSnapshotId(snapshots.ids) < currentId) {
            ...
            snapshots.factors[snapshots.ids.length-1] = factors;
            snapshots.refactorCounts[snapshots.ids.length-1] = refactorCounts;
        }
    }
    ...
    {
        LibDTOSSnapshot.BalanceSnapshots memory snapshot_ =  accountBalanceSnapshots[account];
    ...
    function getAccountBalanceSnapsBySnapshotId(uint256 snapshotId, address account) public view override
        returns (uint256, uint256, uint256, uint256)
    {
        LibDTOSSnapshot.BalanceSnapshots storage snapshot_ =  accountBalanceSnapshots[account];
