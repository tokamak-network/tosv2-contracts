...
contract sOlympus is IsOHM, ERC20Permit {
    ...
    constructor() ERC20("Staked TOS", "LTOS", 18) ERC20Permit("Staked OHM") {
    ...
    // do this last
    function initialize(address _stakingContract, address _treasury, address _tos) external {
        require(msg.sender == initializer, "Initializer:  caller is not initializer");
    ...
    function rebasebyStaker(uint256 profit_, uint256 epoch_) public override onlyStakingContract returns (uint256) {
        uint256 rebaseAmount;
        uint256 circulatingSupply_ = circulatingSupply();
        if (profit_ == 0) {
            emit LogSupply(epoch_, _totalSupply);
