...
describe("TOSv2 Bond Market V1.5", function () {
  ...
  describe("Deploy 4.BonusRateLockUpA.test ", () => {

    it("deploy 4.BonusRateLockUpA.test ", async () => {
  ...
  describe("Set DiscountedRateLockUp ", () => {
    ...
    it("getRatesByWeeks1 : 50 ", async () => {

      let index = 1;
      let weeks = 50;

      let rate = await discountRateLockUp.connect(admin1).getRatesByWeeks1( weeks );
      console.log('rate',rate)

      // expect(rate).to.be.eq(discountSets[index].rates[weekIndex]);
    })
    it("getRatesByWeeks2 : 50 ", async () => {

      let index = 1;
      let weeks = 50;

      let rate = await discountRateLockUp.connect(admin1).getRatesByWeeks2();
      console.log('rate',rate)
