...
describe("TOSv2 Bond Market V1.5", function () {
  ...
  function setBonusRate(bonusRate,  intervalWeeks) {
    ...
    console.log('set.encoded', set.rates);
  ...
  describe("Deploy DiscountedRateLockUpBytes ", () => {

    it("deploy DiscountedRateLockUpBytes ", async () => {
      let factory = await ethers.getContractFactory("BonusRateLockUpBytes")
  ...
  describe("Set DiscountedRateLockUpBytes ", () => {
    it("Set discount datas : interval weeks is 13 weeks ", async () => {
      ...
      let tx = await discountRateLockUp.connect(admin1).createBonusRates(
        set.intervalWeeks,
        set.encoded
      );
      await tx.wait();
    ...
    it("getRatesInfo", async () => {
      ...
      console.log("getRatesInfo", DiscountedRates);
