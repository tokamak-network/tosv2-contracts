...
const fs = require('fs');
const info_simulation_mainnet = require("./info_simulation_mainnet");
...
/////////////////////////////////////
  ...
  describe("#0. pre check migration contract ", () => {

    it("#0-1. balance  ", async () => {
      let totalSupply = await tosContract.totalSupply();
      console.log('totalSupply',ethers.utils.formatEther(totalSupply), "TOS");

      let balanceLockTOS = await tosContract.balanceOf(lockTOSProxyAddress);
      console.log('lockTos balanceOf',ethers.utils.formatEther(balanceLockTOS), "TOS");

      let balanceBonder = await tosContract.balanceOf(deployed.bond);
      console.log('Bonder balanceOf',ethers.utils.formatEther(balanceBonder), "TOS");

      let balanceStake = await tosContract.balanceOf(deployed.stake);
      console.log('Stake balanceOf',ethers.utils.formatEther(balanceStake), "TOS");

      let balanceTreasury = await tosContract.balanceOf(deployed.treasury);
      console.log('Treasury balanceOf',ethers.utils.formatEther(balanceTreasury), "TOS");

      let stakingPrincipal = await stakingProxylogic.stakingPrincipal();
      console.log('staking Principal',ethers.utils.formatEther(stakingPrincipal), "TOS");

    })

    it("#0-2. sum of amounts of lockTOS  ", async () => {
      ...
      if(addLockTosInfos.amounts == null) return;
      ...
      console.log('the sum of amount ( migration data ) ',ethers.utils.formatEther(sum), "TOS");
    })
